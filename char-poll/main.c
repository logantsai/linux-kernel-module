...
static unsigned int dev_poll(struct file *filp, poll_table *wait)
{
	...
	/* This function will not block and add wait_queue into a poll_table.
		The poll_table contain all wait_queue that will change polling status, such as
		r_wait and w_wait.
	*/
	...
	if (devp->current_len != 0) {
		mask |= POLLIN | POLLRDNORM;    /* LDD3 6.3 to check the meaning of flag. */
	}


	/* Have space can write. */
	if (devp->current_len != GLOBALMEM_SIZE) {
